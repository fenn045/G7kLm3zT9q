...
def get_text_from_documents(docs):
    """Extract text from uploaded documents (PDF, DOCX, TXT, XLSX, CSV)."""
    text = ""
    try:
        for file in docs:
...
def main():
    ...
    st.header("Chat bot phân tích tài liệu PDF :robot_face:")
    ...
    with st.sidebar:
        ...
        pdf_docs = st.file_uploader(
            "Tải tài liệu lên (PDF, DOCX, TXT, XLSX, CSV)",
            accept_multiple_files=True,
            type=["pdf", "docx", "txt", "xlsx", "csv"]
        )
        if st.button("Phân tích tài liệu"):
            if not pdf_docs:
                st.error("Vui lòng tải tài liệu PDF của bạn lên.")
                return
            with st.spinner("Đang xử lý..."):
                ...
                if raw_text:
                    ...
                    if text_chunks:
                        ...
                        st.error("Kiểm tra lại nội dung trong tài liệu PDF.")
                else:
